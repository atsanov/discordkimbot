...
SPAM_THRESHOLD = 30
SPAM_COUNT = 6
LONG_TEXT_LENGTH = 800  # 長文もタイムアウト対象
...
SOVIET_IMAGES = [
    ...
    # 追加画像
    "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Yuri_Gagarin_in_1961.jpg/120px-Yuri_Gagarin_in_1961.jpg",
    "https://upload.wikimedia.org/wikipedia/commons/thumb/1/16/Vladimir_Lenin_at_work_1920s.jpg/120px-Vladimir_Lenin_at_work_1920s.jpg"
]
...
# ==================== 宣伝ボタン設置 ====================
@bot.tree.command(name="宣伝設置", description="管理者: 宣伝ボタン設置")
@app_commands.describe(channel="宣伝送信先チャンネル", log_channel="ログチャンネル")
async def setup_promo(interaction: discord.Interaction, channel: discord.TextChannel, log_channel: discord.TextChannel):
    if not is_admin(interaction.user):
        await interaction.response.send_message("❌ 権限なし", ephemeral=True)
        return

    class PromoView(View):
        @discord.ui.button(label="宣伝送信", style=discord.ButtonStyle.primary)
        async def promo(self, button, i: discord.Interaction):
            # 入力欄モーダル
            class PromoModal(Modal, title="宣伝内容入力"):
                content_input = TextInput(label="宣伝内容", style=discord.TextStyle.paragraph)

                async def on_submit(self, modal_interaction: discord.Interaction):
                    await channel.send(self.content_input.value)
                    await log_channel.send(f"📢 {modal_interaction.user} が宣伝送信")
                    await modal_interaction.response.send_message("✅ 宣伝送信完了", ephemeral=True)

            await i.response.send_modal(PromoModal())

    await interaction.response.send_message(f"{channel.mention} に宣伝ボタン設置完了", view=PromoView())
...
async def on_message(message):
    ...
    long_text_spam = len(message.content) >= LONG_TEXT_LENGTH
        ...
        async with session.get(url) as resp:
            ...
            if "articles" in data:
                for art in data["articles"]:
                    embed = discord.Embed(title=art["title"], url=art["url"], description=art.get("description", ""), color=0x00ff00)
                    await interaction.channel.send(embed=embed)
            else:
                await interaction.response.send_message("ニュース取得に失敗しました", ephemeral=True)
