...
# ==================== /help コマンド ====================
@bot.tree.command(name="help", description="Botのヘルプを表示")
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(title="🤖 Botヘルプ", color=0x00ff00)
    embed.add_field(name="!yaju [ユーザーID] [回数]", value="DMに||||を送信（回数指定可能）", inline=False)
    embed.add_field(name="/画像", value="ソ連画像をランダム表示", inline=False)
    embed.add_field(name="/ping", value="動作確認", inline=False)
    embed.add_field(name="/ロール付与 / /ロール削除", value="管理者用ロール管理", inline=False)
...
async def yaju(ctx, target_id: int = None, count: int = 1):
    ...
    if target_id:
        try:
            ...
            await ctx.send(f"✅ {target.display_name} に DM 送信完了")
        except:
            await ctx.send("❌ DM 送信失敗")
    else:
...
async def on_message(message):
    ...
    if any(link in message.content for link in ["discord.gg", "bit.ly", "tinyurl.com"]):
    ...
    if block_required:
        try:
            ...
            class UnTimeoutView(View):
                ...
                async def untout(self, i: discord.Interaction, button: Button):
